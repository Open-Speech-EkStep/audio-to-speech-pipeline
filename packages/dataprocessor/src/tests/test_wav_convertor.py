...
from src.scripts.wav_convertor import convert_to_wav
class TestWavConvertor(unittest.TestCase):
    INPUT_FILE_EXTENSION = 'mp4'
    OUTPUT_FILE_EXTENSION = '.wav'

    def setUp(self):
        super(TestWavConvertor, self).setUp()
    ...
    def test_convert_to_wav_should_return_list_as_output(self):
        self.assertEqual(type(convert_to_wav(self.input_file_dir, self.output_file_dir)), type(""))
    ...
    def test_convert_to_wav_return_should_return_same_number_of_items_as_many_input_mp4_files(self):
        ...
        self.assertEqual(len([convert_to_wav(self.input_file_dir, self.output_file_dir)]), len(input_file_list))

    def test_convert_to_wav_should_return_output_file_with_wav_extension(self):
        input_file_list = glob.glob(self.input_file_dir + '/*.' + self.INPUT_FILE_EXTENSION)
        if len(input_file_list) > 1:
            self.assertTrue(convert_to_wav(self.input_file_dir, self.output_file_dir)[0].endswith(
    ...
    def test_convert_to_wav_should_call_the_subprocess_to_convert_mp4_to_wav(self, mock_subprocess_call):
        convert_to_wav(self.input_file_dir, self.output_file_dir)
