...
class TestAudioClipperSrtPreprocessor(unittest.TestCase):
    ...
    def setUp(self):
        super(TestAudioClipperSrtPreprocessor, self).setUp()
...
class TestAudioClipperWithFFMEG(unittest.TestCase):
    OUTPUT_FILE_EXTENSION = '.wav'
    METADATA_COLUMN = 'utterances_file_list'

    def setUp(self):
        super(TestAudioClipperWithFFMEG, self).setUp()
        self.ca = AudioClipper()
        self.srt_file_path = 'src/tests/test_resources/input/test.srt'
    ...
    @mock.patch('pydub.AudioSegment.from_wav')
    def test_clip_audio_with_ffmeg_should_call_the_from_wav_method_once_for_every_call(self, mock_audio_segment):
        list_objs = self.ca.preprocess_srt(self.srt_file_path)
        self.ca.clip_audio_with_ffmeg(list_objs, self.audio_file_path,
                                      self.output_file_dir)
        self.assertEqual(mock_audio_segment.call_count, 1)
