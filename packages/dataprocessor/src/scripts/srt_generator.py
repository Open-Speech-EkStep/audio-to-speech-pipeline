...
class SRTGenerator(object):
    ...
    def convert_to_wav_backup(self, input_dir, output_dir=None, ext='mp4'):
    ...
    def fit_single(self, bin_size, input_file_path, audio_extn, output_file_path=None, bucket_name=None,
                   dump_response=False, dump_response_directory=None, job_mode="cluster"):

        print("Processing single audio file...")
        print("Initiating input raw file conversion to wav format...")
        # srt_output_file_paths = []
        print("****input_file_path in fit_single**** ", input_file_path)
        ...
        for filepath in output_file_paths:
            ...
            obj_gcs.upload_to_gcs(bucket_name, filepath, os.path.join(input_file_path, file_name) + ".wav",
                                  is_directory=False)
            wav_file_path = os.path.join("gs://", bucket_name, input_file_path, file_name + ".wav")
    # else:
        #     for filepath in output_file_paths:
        #         file_name = filepath.split('/')[-1].split('.')[0]
        #         wav_file_path = os.path.join(input_file_path, file_name + ".wav")

        print("Initiating conversion of wav file to text using speech to text API")
        response = self.call_speech_to_text(wav_file_path)
        ...
        print("Subtitle file saved successfully at: {}".format(
            os.path.join(current_working_directory, output_file_path)))
        # srt_output_file_paths.append(output_file_path)

        return output_file_path,output_file_paths[0]

    def fit_dir(self, bin_size,bucket_name,input_file_dir, audio_extn, output_file_dir=None, dump_response=False,
        ...
        print("********input_file_dir in fir_dir*******", input_file_dir)
        audio_id_file_dirs = glob.glob(input_file_dir + '/*')
        print("********audio_id_file_dirs in fir_dir*******", audio_id_file_dirs)
        for audio_id_dir in audio_id_file_dirs:
            if output_file_dir is None:
                print("**********in file dir input file**********")

                srt_local_path,audio_local_path = self.fit_single(bin_size, audio_id_dir, audio_extn, dump_response=dump_response,
                                             dump_response_directory=dump_response_directory,bucket_name = bucket_name, job_mode="local")
                srt_file_paths.append(srt_local_path)
                wav_file_paths.append(audio_local_path)

            else:
                video_name = audio_id_dir.split('/')[-1].split('.')[0]
                output_file_name = output_file_dir + '/' + video_name + '.srt'

                srt_local_path,audio_local_path = self.fit_single(bin_size=bin_size, input_file_path=audio_id_dir, audio_extn=audio_extn,
                                             output_file_path=output_file_name,
                                             dump_response=dump_response,
                                             dump_response_directory=dump_response_directory,
                                             bucket_name = bucket_name,
                                             job_mode="local")
                srt_file_paths.append(srt_local_path)
                wav_file_paths.append(audio_local_path)

        return srt_file_paths,wav_file_paths
    ...
    def generate_subtitle_new(self, response, bin_size=10):
        ...
        for i in np.arange(start_sec, end_sec, bin_size):
            ...
            for j in list_words:
                ...
                if (local_time_start >= window_start and local_time_end <= window_end) or (
                        local_time_start < window_end and local_time_end > window_end):
        ...
        for key, value in new_windows.items():
            ...
            transcriptions.append(
                srt.Subtitle(index, datetime.timedelta(0, value[0].start_sec, value[0].start_nano_sec * 0.001),
                             datetime.timedelta(0, value[len(value) - 1].end_sec,
                                                value[len(value) - 1].end_nano_sec * 0.001), " ".join(transcript)))
            index = index + 1
                    ...
                    try:
                        ...
                        if word_end_sec < end_sec:
                            ...
                            if flag == 1:
                                ...
                                word_start_microsec = result.alternatives[0].words[
                                                          i + 2].start_time.nanos * 0.001  # 0.001 to convert nana -> micro
