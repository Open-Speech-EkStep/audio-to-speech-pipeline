...
from audio_processing import generate_hash
...
class AudioProcessorTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.audio_processer = AudioProcessor(self.postgres_client, self.gcp_instance,self.audio_commons,self.catalogue_dao)
    ...
    @mock.patch('audio_processing.audio_processer.get_hash_code_of_audio_file')
    def test__process_audio_id_when_file_is_duplicate_should_not_call_SNR(self,generate_hash_mock):
        ...
        self.audio_processer.process_audio_id("audio_id", "source", "extension","file_name")
        self.assertEqual(self.audio_processer.fs_interface.move.call_count,2)
        self.assertEqual(self.audio_processer.fs_interface.download_file_to_location.call_count,2)
        self.assertEqual(self.audio_commons["chunking_conversion"].convert_to_wav.call_count,0)
        self.assertEqual(self.audio_processer.fs_interface.upload_folder_to_location.call_count,0)

        self.assertEqual(self.catalogue_dao.check_file_exist_in_db.call_count,1)
        self.assertEqual(self.catalogue_dao.upload_file.call_count,0)

        self.catalogue_dao.check_file_exist_in_db.assert_called_with('file_name', '12334')


    @mock.patch('audio_processing.audio_processer.get_hash_code_of_audio_file')
    def test__process_audio_id_when_file_is_not_duplicate_should_call_SNR(self,generate_hash_mock):

        self.catalogue_dao.check_file_exist_in_db.return_value = False
        generate_hash_mock.return_value="12334"
        self.audio_commons["chunking_conversion"].convert_to_wav.return_value = "output_file_path",True
        ...
        self.audio_processer.process_audio_id("audio_id", "source", "mp4","file_name.mp4")
        self.assertEqual(self.audio_processer.fs_interface.move.call_count,2)
        self.assertEqual(self.audio_processer.fs_interface.download_file_to_location.call_count,2)
        self.assertEqual(self.audio_processer.fs_interface.upload_folder_to_location.call_count,2)
        self.assertEqual(self.audio_commons["chunking_conversion"].convert_to_wav.call_count,1)
        self.assertEqual(self.catalogue_dao.upload_file.call_count,1)

        self.assertEqual(self.catalogue_dao.check_file_exist_in_db.call_count,1)
        self.catalogue_dao.check_file_exist_in_db.assert_called_with('file_name.mp4', '12334')
