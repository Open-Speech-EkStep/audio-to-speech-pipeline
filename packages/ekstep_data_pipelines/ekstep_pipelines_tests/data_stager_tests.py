...
from data_marker.data_stager import DataStager

sys.path.insert(0, '..')


class DataStagerTests(unittest.TestCase):

    def setUp(self):
        self.postgres_client = Mock()
        self.gcp_instance = Mock()
        self.data_stager = DataStager(self.postgres_client, self.gcp_instance )
