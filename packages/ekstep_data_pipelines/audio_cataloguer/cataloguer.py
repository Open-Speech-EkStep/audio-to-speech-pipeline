...
class AudioCataloguer(BaseProcessor):
    ...
    def process(self, **kwargs):
        ...
        """
        Logger.info(
            "moving data from staging to mapping table if speaker is not identified"
        )
    ...
    def update_mapping_table_when_speaker_is_null(self):
        ...
        self.set_isnormalized_flag(processed_audio_ids)
    ...
    def set_isnormalized_flag(self, audio_ids, is_nested=False):
        ...
        if is_nested:
            audio_id_list = (
                f"({','.join([f'{audio_id[0]}' for audio_id in audio_ids])})"
            )
    ...
    def update_utterance_in_mapping_table(self):
        ...
        for onefile in all_data:
            ...
            for utterance in utterance_list:
                language_confidence_score = json.dumps(
    ...
    def copy_data_from_media_metadata_staging_to_media(self):
        max_date_result = self.data_processor.execute_query(
            MAX_LOAD_DATE_FOR_MEDIA_QUERY
        )
        max_datetime = max_date_result[0][0]
        self.data_processor.execute_update(
            INSERT_INTO_MEDIA_TABLE_QUERY, max_datetime=max_datetime
        )
