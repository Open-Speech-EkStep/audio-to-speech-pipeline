...
INSERT_INTO_MEDIA_TABLE_QUERY = "INSERT INTO media(audio_id,raw_file_name,total_duration,title," \
...
INSERT_UNIQUE_SPEAKER_QUERY = "INSERT INTO speaker(speaker_name,source,gender,mother_tongue," \
...
GET_AUDIO_ID_QUERY = "SELECT media_metadata_staging.audio_id FROM media_metadata_staging" \
                     " where is_normalized = false and speaker_name is not null"

DEFULT_QUERY_FOR_INSERT_INTO_MAPPING_TABLE = "insert into media_speaker_mapping(audio_id, " \
...
GET_SPEAKER_ID_QUERY = "select speaker_id from speaker s JOIN media_metadata_staging b on " \
                       "s.speaker_name = b.speaker_name \
where b.audio_id = :audio_id;"

FETCH_QUERY_WHERE_SPEAKER_IS_NULL = "select audio_id ,utterances_files_list,load_datetime " \
