...
def encoder(source_dir, source_dir_pattern, embed_file_name):
    ...
    print("file_paths type:" + str(type(file_paths[0])))

    processed_wavs = Parallel(n_jobs=-1)(
        delayed(preprocess_wav)(i) for i in tqdm(file_paths)
    )
    vocoder = VoiceEncoder()

    encodings = Parallel(n_jobs=-1)(
        delayed(vocoder.embed_utterance)(i) for i in tqdm(processed_wavs)
    )
...
if __name__ == "__main__":
    encoder(
        source_dir="/home/anirudh/sdb/audio_speaker_clustering/processed_ldcil_data/",
