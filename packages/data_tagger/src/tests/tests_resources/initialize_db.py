def create_db(c, conn):
    ...
    c.execute(
        "insert into media_speaker_mapping(audio_id, speaker_id, clipped_utterance_file_name, clipped_utterance_duration,load_datetime) values (123,5,'some_name',3,123445),(123,6,'some_name2',3,123)"
    )
    # Table: media
    # Create table media -------
    c.execute(
    ...
    c.execute(
        "insert into media(audio_id,raw_file_name,load_datetime) values (1234 ,'somefile',julianday('now'))"
    )
    # Table: speaker
    ...
    c.execute(
        "INSERT INTO media_metadata_staging(raw_file_name ,duration ,title , speaker_name ,audio_id,cleaned_duration,num_of_speakers,language,type,source,experiment_use ,utterances_files_list,load_datetime ) VALUES \
...
def find_used_speaker_count(c):
    count = c.execute(
        "select count(*) from media_speaker_mapping where speaker_exp_use_status = 'true'"
    ).fetchall()
    return count[0][0]
