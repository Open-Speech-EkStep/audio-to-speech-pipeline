...
from .db_query import (
    GET_NEW_SPEAKER,
    GET_EXPERIMENT_ID,
    GET_ALL_SPEAKER_ID_FROM_GIVEN_EXP,
    INSERT_NEW_EXPERIMENT,
    INITIAL_TEXT_OF_INSERT_QUERY,
    GET_NEW_SPEAKER,
    GET_UTTERANCES_OF_GIVEN_EXP,
    GET_UTTERANCES_OF_NEW_USER,
    INITIAL_TEXT_OF_UPDATE_QUERY,
    GET_ALL_DATA_OF_CURRENT_EXP,
    GET_NEW_SPEAKER_WITH_SOURCE,
    UPDATE_SOURCE_TABLE_WITH_DURATION,
    INSERT_NEW_ROW_FOR_EXISTING_SOURCE,
    GET_EXISTING_SOURCE,
)
...
class ExperimentDataTagger:
    def get_existing_experiment_id(self, connection):
        ...
        exp_id = connection.execute(query, exp_name=existing_experiment_name).fetchall()
        return exp_id[0][0]
    ...
    def insert_and_get_exp_id(self, connection):
        ...
        get_experiment_id = connection.execute(
            query, name=experiment_name, desc=experiment_description
        ).fetchall()
    ...
    def for_new_experiment_with_using_existing_exp(
        self, connection, current_exp_id, trans
    ):
        ...
        get_all_speaker_id = connection.execute(
            query,
            time=duration_per_speaker_in_second,
            id=existing_experiment_id,
            require_speaker_from_existing_exp=number_of_speaker_from_existing_experiment,
        ).fetchall()
    ...
    def tag_existing_source(self, connection, exp_id):
        ...
        for source in existing_source_name:
            all_data_of_existing_source = self.get_all_source_data(connection, source)
            self.insert_new_row(connection, all_data_of_existing_source, exp_id)
            all_path = obj_gcs.list_blobs_in_a_path(
                bucket_name, integration_path + source + "/"
            )
            for path in all_path:
    ...
    def insert_new_row(self, connection, source_data, exp_id):
        ...
        connection.execute(
            ...
            num_of_speaker=source_data[1],
            total_duration=source_data[2],
            cleaned_duration=source_data[3],
            num_of_audio=source_data[4],
            exp_id=exp_id,
        )
    ...
    def create_insert_query(
        self, get_all_utterances, speaker_id, current_exp_id, clean_duration=0
    ):
    ...
    def find_all_speaker(self, connection):
        if len(source) > 1 and not isinstance(source, int):
            ...
            results = connection.execute(
                query,
                time=duration_per_speaker_in_second,
                require_speaker=require_new_speaker,
                source_name=source,
            ).fetchall()
            print(results)
            return results
    ...
    def get_utterances_for_existing_experiment_data(
        self, connection, speaker_id, exp_id
    ):
        query = text(GET_UTTERANCES_OF_GIVEN_EXP)
        results = connection.execute(
            query, speaker_id=speaker_id, exp_id=exp_id
        ).fetchall()
        return results

    def get_utterances(self, connection, speaker_id):
        query = text(GET_UTTERANCES_OF_NEW_USER)
        results = connection.execute(query, speaker_id=speaker_id).fetchall()
        return results

    def clean_duration_threshold(
        self,
        get_all_utterances,
        current_exp_id,
        duration_per_speaker_in_second,
        clean_duration=0,
    ):
        ...
        for utterance in get_all_utterances:
            clean_duration += utterance[2]
            update_utterances_query = update_utterances_query + f"'{utterance[1]}',"
            if clean_duration >= duration_per_speaker_in_second:
    ...
    def create_qurey_and_update_table_for_new_speaker(self, connection, current_exp_id):
        ...
        for speaker_id in get_all_speaker_id:
            update_query_for_all_speaker = (
                update_query_for_all_speaker + f"{speaker_id[0]},"
            )
    ...
    with open(metadata_output_path + "/all_tagged_data.csv", "w") as f:
        ...
        for index, item in enumerate(result):
            if index % 5000 == 0 and index != 0:
                f.close()
                f = open(f"{metadata_output_path}/all_tagged_data{index}.csv", "w")
                out = csv.writer(f)
                out.writerow(
                    [
                        "audio_id",
...
def get_variables(config_file_path):
    ...
    validate_input(
        num_speakers, duration_per_speaker_in_second, experiment_name, using_source
    )
    validate_existing_exp_input(
        use_existing_experiment_data,
        number_of_speaker_from_existing_experiment,
...
def validate_input(
    ...
    if (
        not isinstance(num_speakers, int)
        or not isinstance(duration_per_speaker_in_second, int)
        or not num_speakers
        or num_speakers <= 0
        or not duration_per_speaker_in_second
        or duration_per_speaker_in_second <= 0
        or len(experiment_name.strip()) <= 0
    ):
...
def validate_existing_exp_input(
    use_existing_experiment_data,
    number_of_speaker_from_existing_experiment,
    existing_experiment_name,
    using_source,
    ...
    if use_existing_experiment_data == True:
        if (
            len(existing_experiment_name.strip()) <= 0
            or not isinstance(number_of_speaker_from_existing_experiment, int)
            or not number_of_speaker_from_existing_experiment
            or number_of_speaker_from_existing_experiment <= 0
        ):
            print("using_source is ", using_source)
            if not using_source:
                raise ValueError(
...
if __name__ == "__main__":
    ...
    try:
        ...
        obj_gcs.make_directories(
            os.path.join(current_working_directory, metadata_output_path)
        )
        if num_speakers > 0:
