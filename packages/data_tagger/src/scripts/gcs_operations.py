...
class CloudStorageOperations:
    ...
    def download_to_local(
        ...
        if is_directory:
            ...
            blobs = list(
                storage_client.list_blobs(bucket_name, prefix=source_blob_name)
            )
                ...
                if (not blob.name.endswith("/")) & (
                    ...
                    print(
                        "Downloading blob {}/{} to local directory: {}: ".format(
                            bucket_name, blob.name, destination
                        )
                    )
            ...
            print(
                "File {}/{} downloaded to destination directory {} successfully".format(
                    bucket_name, source_blob_name, destination_directory
                )
            )
    ...
    def upload_to_gcs(self, bucket_name, source, destination_blob_name, is_directory):
        ...
        if is_directory:
            ...
            for file in files:
                ...
                print(
                    "Uploading files from source: {} to destination: {}/{} ".format(
                        src_file, bucket_name, blob.name
                    )
                )
            ...
            print("Running in FILE mode...")
            print(
                "Uploading file from source: {} to destination: {}/{} ".format(
                    source, bucket_name, destination_blob_name
                )
            )
            blob = bucket.blob(destination_blob_name)
            blob.upload_from_filename(source)
            print(
                "File uploaded successfully to {}/{}".format(
