...
def move_metadata_file(source):
    global bucket_name
    source_file_name = source + "_audio_id.txt"
    destination_blob_name = tobe_processed_path + source + '/audio_id/' + get_audio_id() + '/' + source + "_audio_id.txt"
    if os.path.isfile(source_file_name):
        upload_blob(bucket_name, source_file_name, destination_blob_name)
        os.remove(source_file_name)
    else:
        create_empty_file(source)
        move_metadata_file(source)
...
def get_files_from_landing_zone(source):
    ...
    all_blobs = list_blobs_in_a_path(bucket_name, source_landing_path + source + delimiter)
    try:
        for blob in all_blobs:
            ...
            if file_extension in [expected_file_extension, expected_file_extension.swapcase()]:
                ...
                if batch_count > 0:
                    ...
                    source_file_name = get_files_path_with_no_prefix(source_landing_path + source, file_name)
                    ...
                    if (check_if_meta_data_present(source_landing_path + source, metadata_file_name)):
                        print("Meta file {} is present".format(metadata_file_name))
                        # meta_data_flag = True
                        move_blob(bucket_name, source_file_name, bucket_name, destination_file_name)
                        move_blob(bucket_name, source_meta_file_name, bucket_name, destination_meta_file_name)
                        update_metadata_file(source, audio_id)
                    else:
                        print("Meta file {} is not present,Moving to error....".format(metadata_file_name))
                        error_destination_file_name = get_files_path_with_no_prefix(error_landing_path + source,
                                                                                    file_name)
                        move_blob(bucket_name, source_file_name, bucket_name, error_destination_file_name)
...
def get_audio_ids(source, **kwargs):
    get_variables()
    audio_file_ids = json.loads(Variable.get("audiofileids"))
    audio_id_source_path = get_audio_id_path(tobe_processed_path, source)
    latest_audio_id_path = audio_id_source_path + get_latest_file_from_bucket(audio_id_source_path)
