...
class AnalyseSpeakersTests(unittest.TestCase):
    ...
    @mock.patch('ekstep_data_pipelines.audio_analysis.speaker_analysis.create_embeddings.encoder')
    @mock.patch('ekstep_data_pipelines.audio_analysis.speaker_analysis.create_embeddings.encoder')
    @mock.patch('ekstep_data_pipelines.audio_analysis.speaker_analysis.speaker_clustering.create_speaker_clusters')
    @mock.patch('ekstep_data_pipelines.audio_analysis.speaker_analysis.file_cluster_mapping.speaker_to_file_name_map')
    @unittest.skip
    def should_analyse_speakers_for_source(self, speaker_to_file_name_map_mock, create_speaker_clusters_mock, encoder_mock, catalogue_dao):
        ...
        analyse_speakers(embed_file_name, '*/clean/*.wav', source_path, 'test_source', catalogue_dao, 2, 11122, 2, 0.80)
